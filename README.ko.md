...
색상이 들어간 한 줄이 나오고 끝이 `… resets 1:00am` 형태로 끝나면 정상입니다.
...
Claude Code는 컨텍스트 윈도우 잔여량이 약 16.5% 이하로 떨어지면 자동으로 대화를 압축(auto-compact)합니다. `remaining_percentage` 원본을 그대로 쓰면 압축이 일어난 *후에야* 바가 100%에 도달해서 신호로서 의미가 약해집니다.
...
즉, 바가 **100%**를 가리키면 auto-compact가 들어가기 직전이라는 의미입니다 — 컨텍스트 윈도우의 절대 한계가 아닙니다. 실제로 사용자가 체감하는 "압축 시점"과 일치합니다.

Anthropic이 auto-compact 임계값을 바꾸면, `statusline.js` 안의 `AUTO_COMPACT_BUFFER_PCT` 상수만 조정하면 됩니다.
